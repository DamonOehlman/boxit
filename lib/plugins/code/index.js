...
module.exports = function(processor, input, callback) {
    ...
    if (fields.length > 0) {
        var extensionMatch = reExtension.exec(fields[0]),
            extension = extensionMatch ? extensionMatch[1] : 'js',
            language = languageLookups[extension] || extension,
            style = fields[1] || 'default';
        
        getit(fields[0], getOpts, function(err, data) {
            if (! err) {
                // wrap the content in a pre code block
                var content = '<pre><code class="' + language + '">' + data + '</pre></code>';
