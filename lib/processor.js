        ...
        getit(templateFile, { cwd: this.basePath }, function(err, data) {
            if (! err) {
                try {
                    ...
                    // create the template
                    processor.template = new Template(processor, templateOpts);
                }
                catch (e) {
...
Processor.prototype.run = function(opts, callback) {
    ...
    async.series([
        ...
        processor.loadTemplate.bind(processor), 
        function(itemCallback) {
            processor.template.process(processor, items, itemCallback);
        },
        processor.getResources.bind(processor), 
        processor.writeOutput.bind(processor)
    ], function(err) {
        if (callback) {
            callback(err, {
