...
function Processor(targetfile, opts) {
    ...
    this.outputPath = path.resolve(this.basePath, opts.output);
    this.output = '';
    this.items = [];
    
    debug('creating processor, output path = ' + this.outputPath);
...
Processor.prototype.getResources = function(callback) {
    ...
    async.forEachSeries(
        [this.template.downloader, this.downloader], 
        function(item, itemCallback) {
...
Processor.prototype.getTemplateLocation = function(target) {
    var isRemote = getit.isRemote(target),
        templateBase = (target || '').replace(rePackageJson, '');
    ...
    return templateBase.replace(reTrailingSlash, '');
};
...
Processor.prototype.highlight = function(language, style) {
    ...
    this.require('js/highlight.js <= http://yandex.st/highlightjs/7.2/highlight.min.js');
...
Processor.prototype.loadTemplate = function(target, callback) {
    var processor = this,
        templateLocation = this.getTemplateLocation(target) || '',
        templateFile = templateLocation + '/template.json';
        
    if (! templateLocation) return callback(new Error('No template specified'));
    ...
    getit(templateFile, processor._getitOpts(), function(err, data) {
        if (! err) {
            ...
            if (! err) {
                ...
                templateOpts.basePath = templateLocation;
    ...
    for (var key in this.includes) {
        ...
        for (var ii = includes.length; ii--; ) {
            if (! reTagged.test(includes[ii])) {
                ...
                // only include the file if we have a replacer
                // as if we don't it's likely to be a supporting asset (image, font, etc)
                if (replacer) {
...
Processor.prototype.run = function(opts, callback) {
    ...
    async.series([
        ...
        this.loadTemplate.bind(this, this.templateFile), 
...
Processor.prototype.tryRequire = function(targetModule, callback) {
    var mod, 
        err, 
        targetPaths = [this.basePath, this.outputPath, process.cwd()];
    ...
    targetPaths.forEach(function(targetPath) {
        ...
        try {
            mod = require(path.resolve(targetPath, 'node_modules', targetModule));
        }
        catch (e) {
        }
    });
    ...
    if (! mod) {
        err = new Error('Unable to load module "' + targetModule + '", install in the project folder');
    }
