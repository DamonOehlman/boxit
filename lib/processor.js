...
Processor.prototype.expandDirs = function(sources, callback, basePath) {
    var processor = this,
        toRemove = [];
    ...
    async.forEach(
        [].concat(sources),
        function(item, itemCallback) {
            var targetPath = path.resolve(basePath || processor.basePath, item);
...
Processor.prototype.processItems = function(callback) {
    ...
    this.template.load(function(err, data) {
        if (! err) {
            processor.output = handlebars.compile(data)(processor);
        }
...
Processor.prototype.run = function(opts, callback) {
    ...
    async.series([
        this.expandDirs.bind(this, this.sources),
